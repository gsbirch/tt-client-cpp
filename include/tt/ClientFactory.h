    ...
    */
    class ClientFactory {
        ...
        private:
            ...
            class Operation {
                public:
                    explicit Operation(std::function<void()> function, ClientFactory& factory)
                        : function_(std::move(function)), factory_(factory)
                    {
                    }
            ...
            std::deque<client_ptr> waiting;
            
            /** Clients whose sessions are currently happening */
            std::deque<client_ptr> running;
            
            /** An thread pool to run each new client */
